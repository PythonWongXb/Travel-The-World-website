...
from app.admin.forms import LoginForm,PwdForm,AreaForm,ScenicForm,TravelsForm,AdminRegister
...
def admin_register():
    ...
    if form.validate_on_submit():  # 验证提交表单
        data = form.data  # 接收数据
        m = Admin.query.filter_by(name=data["account"]).first()  # 获取用户信息
        if m:
            flash("user existed！", "err")  # 输出错误信息
            return redirect(url_for("admin.login"))  # 调回登录页



        admins = Admin(
            name=data["account"],  # 用户名
            pwd=generate_password_hash(data["pwd"]),  # 对密码加密
        )
        db.session.add(admins)  # 添加数据
        db.session.commit()  # 提交数据
        flash("注册成功！", "ok")  # 使用flask存储成功信息
    return render_template("admin/register_admin.html", form=form)  # 渲染模板
def admin_login(f):
    """
    登录装饰器
    """
